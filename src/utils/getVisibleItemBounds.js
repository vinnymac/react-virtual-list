...
const getVisibleItemBounds = (list, container, items, itemHeight, itemBuffer) => {
  ...
  const listViewTop =  Math.max(0, Math.min(viewTop - listTop)); // top y-coordinate of list that is visible inside container
  const listViewBottom = Math.max(0, Math.min(listHeight, viewBottom - listTop)); // bottom y-coordinate of list that is visible inside container
